...
predictors <- terra::rast("input/predictors.tif")

if(du != 0){
  OCabove <- raster(paste0("output/OC0-10cm_median_", date, ".tif"))
  predictors <- stack(predictors[[c("BATH", "Mud")]], OCabove)
  names(predictors)[[length(names(predictors))]] <- "OC"
  rm(OCabove)
} else {
  OC <- read_sf(paste0("input/OC", du, "_", date, ".shp"))
}
